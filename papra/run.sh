...
if [ "$AI_IS_ENABLED" = "true" ]; then
  ...
  if [ -z "$AI_PROVIDER" ]; then
    echo "[papra-addon] FEHLER: 'ai_is_enabled' ist aktiv, aber 'ai_provider' ist leer."
    echo "[papra-addon] Waehle einen Provider (z.B. openai, anthropic, ollama, ...)."
    exit 1
  fi
  ...
  echo "[papra-addon] KI-Funktionen aktiviert: Provider=$AI_PROVIDER Modell=$AI_DEFAULT_MODEL"
else
